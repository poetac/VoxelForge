...
//
// Lifted from Voxelforge.Airbreathing.Tests/Helpers/SubprocessRunner.cs per
// the parallel-pillar policy. Cross-pillar reuse deferred until rule-of-three
// trigger fires for shared test infrastructure.

using System.Diagnostics;
using System.IO;
using System.Text;

namespace Voxelforge.Marine.Tests.Helpers;

/// <summary>
/// Discovers test-built executables and runs them in xUnit-safe
/// subprocess mode. Covers repo-anchor discovery (walk up to
/// <c>voxelforge.sln</c>) and stdin/stdout piping with a hard timeout.
/// </summary>
public static class SubprocessRunner
{
    ...
    /// then resolve <paramref name="relativeFromRepoRoot"/> beneath it.
    /// </summary>
    ...
    /// <summary>
    /// Probe an exe path that may not exist yet. Returns a sentinel with
    ...
    /// <summary>Spawn an exe with redirected I/O and a wait timeout.</summary>
        ...
        string? workingDirectory = null)
    {
        ...
        if (!exitedInTime)
        {
            try { p.Kill(entireProcessTree: true); } catch { /* race: may have just exited */ }
        }
...
/// <summary>Aggregated result of a sub-process run.</summary>
