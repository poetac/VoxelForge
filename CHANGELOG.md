...
A second adversarial audit (parallel pillar sweeps over marine, electric propulsion, system-integration/economics, and the Wave-1 energy pillars; later rounds added nuclear/CFD, the air-breathing helpers, and the chamber/nozzle geometry) plus targeted probing surfaced more correctness holes that green CI didn't catch. Each fix lands with a fail-on-old / pass-on-new regression test on the cross-platform suites; calibration-laden findings are documented rather than blind-fixed.
