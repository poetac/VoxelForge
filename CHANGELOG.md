...
**Fix:** energy conservation, not a fixed ratio — `NuclearOptimization.GenerateWith` now computes the Brayton tap first (`TryRunBraytonModel`, unchanged) and, for `BimodalMode.Hybrid` only, passes `ReactorThermalPower_MW − brayton.ReactorPowerToBrayton_MW` into a new optional `NtrCycleSolver.Solve` parameter as the propellant-heating power. `ReactorPowerToBrayton_MW` is already capped at the full reactor output inside `BraytonGasLoopSolver.Solve`, so the remainder never goes negative. A fixed 20/80 split was considered and rejected: it would make the design's own `ElectricPowerTarget_kWe` irrelevant to the actual thrust/electric split, and the existing `NTR_BIMODAL_REACTOR_TAP_EXCESSIVE` advisory gate's own design intent ("no thrust headroom remains") already assumes a variable, conservation-style split. `NtrCycleSolver`'s volumetric-heat-flux output is deliberately left reading `design.ReactorThermalPower_MW` directly (not the split propellant-heating power) — it's a reactor-level metric independent of how the heat is subsequently divided between the thrust nozzle and the Brayton loop. Pure-Thrust designs are unaffected (`TryRunBraytonModel` returns null before the split is even considered) and pure-Electric designs are unaffected by design-intent choice, not oversight — their thrust/Isp/c* result fields are NaN'd regardless of the cycle solve's output, so splitting there would only change the otherwise-unobserved `CoreExitTemp_K` feeding the regen-cooling advisory gate for no benefit; issue #77 and its "Pure-Thrust and pure-Electric modes are unaffected" framing scope the fix to Hybrid only.

- New `Voxelforge.Nuclear.Tests/BimodalNtrHybridPowerSplitTests.cs` (7 tests): a white-box wiring proof (`GenerateWith`'s cycle result exactly reproduces a direct `NtrCycleSolver.Solve` call given the reconstructed post-tap power), a conservation proof comparing against a naive unsplit `Solve` call and against an otherwise-identical Thrust-mode design (both fail on pre-fix code, where Hybrid and "full reactor power" were identical), a guard that `VolumetricHeatFlux_MWm3` stays keyed to the full reactor power, a capped-tap edge case (electric target exceeding the reactor's total output degrades to a finite cold-flow result, not NaN/crash), and Thrust/Electric bit-identity guards against a bare `Solve` call.
- Existing `BimodalNtrGateTests.cs` / `Fixtures/BimodalNtrSp100Fixture.cs` Hybrid-mode fixtures needed no recalibration: none pin an exact thrust/Isp/temperature magnitude — they check signs, ranges, Brayton-only outputs (unaffected, since `BraytonGasLoopSolver.Solve` never depended on the cycle result), 2nd-law/ceiling invariants, or self-consistency (determinism, round-trip), all of which remain true under a reduced-but-still-positive propellant-heating power.
- Drops the `physics-cascade-status.md` § Documented gaps entry and the ROADMAP known-gaps burn-down line (Crocco #76, HET #78, VASIMR #79 remain — calibration decisions, not data-plumbing/design-intent fixes already made here). Updates criterion 4's live gap list.
...
- Drops the `physics-cascade-status.md` § Documented gaps entry and this item's ROADMAP known-gaps burn-down line (Crocco, HET, Bimodal-Hybrid, and VASIMR remain on that list — calibration decisions, not data-plumbing fixes, so out of scope here). Updates criterion 4's live gap list.
...
**Model:** replaced with West's number, `MEP = Wn · P_mean · τ · fluidFactor` where `τ = (T_hot−T_cold)/(T_hot+T_cold)`. `Wn = 0.25` — verified via WebSearch against the documented average for a wide variety of Stirling engines (range up to ≈0.35 for well-engineered high-ΔT designs; the companion "Beale number" omits the τ term and clusters 0.11-0.15 for the same population) — this correction matters: an earlier draft of this issue's own guide comment (Sprint A.125) had mis-stated the range as "0.011-0.015", roughly 10× low, which would have just reintroduced a different order-of-magnitude bug in the opposite direction had it been used blind. Cross-checked against the GM/NASA GPU-3 rhombic-drive reference engine's published high-power helium operating point (≈4.13 MPa, 41.72 Hz, ~120 cm³ swept volume, ~650-700°C hot / ~13°C cold, 3958 W) — back-solving for Wn at that specific point gives ≈0.35-0.37, consistent with the documented range and confirming 0.25 is a conservative central estimate, not an overfit to one data point.

- `StirlingSolver.cs`: `IndicatedWorkFractionOfMeanPressure` (0.5 constant) replaced with `WestNumber` (0.25) + the τ term computed at the call site. Stale header/doc comments describing the old model and its accuracy caveat corrected throughout (`StirlingSolver.cs`, `StirlingResult.cs`).
- New `Voxelforge.Core.Tests/StirlingSolverTests.cs` — the first Linux fixture for the Stirling family (previously the only Wave-1 pillar without one). Exact closed-form assertions (hand-derived, cross-checked via a Python mirror of the exact formula chain — not read off a test run), a fail-on-old proof (a barely-warm design produces power at least 20× smaller than a high-ΔT design at the same pressure/volume/frequency — impossible under the old ΔT-independent constant), monotonicity invariants (P_indicated scales linearly with P_mean and f), a 2nd-law invariant (η_indicated ≤ η_Carnot always), and `ValidateSelf`/null-argument guard tests.
- Drops the `physics-cascade-status.md` § Documented gaps entry, the `LIMITATIONS.md` "Validated free-piston Stirling output" caveat, and the ROADMAP known-gaps burn-down line. Updates ROADMAP criterion 2 to 23/23 SATISFIED, the Done section's "Stirling deferred" note, criterion 4's gap list, and issue #10's Wave-1 cluster-anchor fixture list.

### Sprint A.129 — Fix #80: `SobolSequence` Joe-Kuo direction numbers retranscribed + verified

Closes issue #80. The baked Joe-Kuo direction-number table (`s_polynomials`/`s_mValues`, dims 1-7) mis-transcribed the genuine `new-joe-kuo-6.21201` reference table — computationally re-derived and cross-checked against two independent mirrors of the original web.maths.unsw.edu.au/~fkuo/sobol/ source (github.com/diku-dk/sobol-futhark and github.com/joe-kuo/sobol_data, byte-identical for d=2..9), the actual scope of the corruption is larger than originally diagnosed: **5 of the 7 baked dimensions** (not just the one dim-3 row-fusion found by inspection) decoded to non-primitive polynomials over GF(2) — verified via a brute-force GF(2) multiplicative-order check (Python during authoring; the identical algorithm now lives in the new test). Only dims 1-2 were accidentally valid, because they happened to coincide with what should have been source d=3/d=4's values (the whole table was shifted/fused, not just one entry).

Both mis-stated header comments claiming the dims≥8 fallback is "Halton-style" are corrected to describe what it actually computes (a pseudo-stratified hash-based sequence, neither genuine Sobol nor Halton) — issue #80's secondary finding.

- New `Voxelforge.Core.Tests/SobolSequencePropertyTests.cs`: every baked `m_i` is odd, every `m_i < 2^(i+1)` (1-indexed literature convention), and every `(s, a)` pair decodes to a primitive GF(2) polynomial — checked against the actual production table directly (`s_polynomials`/`s_mValues` changed from `private` to `internal`, a friend-assembly-only visibility change, so the test can't drift from a hand-copied duplicate). Fails against the pre-fix table (5 of 7 dims non-primitive) — genuine fail-on-old.
- **Windows-leg risk assessed, not executed**: read (not run) every Windows-only `Voxelforge.Tests` file touching `SobolSequence`/`MultiChainOptimizer` warmup — none pin exact values tied to the specific table (determinism/structural/loose-bound checks only); `SobolSensitivityTests.cs` uses plain `Random`, unrelated despite the name. Logged in `physics-cascade-status.md` § Windows-leg validation pending for the actual first run.
- Drops the `physics-cascade-status.md` § Documented gaps entry and its ROADMAP known-gaps burn-down line.

### Sprint A.128 — Fix #82: `ACOUSTIC_DAMPER_OVERSIZED` checks true cavity volume

Closes issue #82. `AcousticDamperResult` gains `TotalVolume_mm3`, stamped by `AcousticDamper.Evaluate` itself from the live `AcousticDamperConfig` (which already computes `TotalVolume_mm3` for both Helmholtz and quarter-wave geometries) before the config goes out of scope. `EmitAcousticDamperOversized` (`RocketGates.cs`) drops the throwaway zero-geometry `AcousticDamperConfig` reconstruction — provably dead code (`_ = config;`), since every field was hardcoded to 0 and could never make the volume check fire — and instead checks `damper.TotalVolume_mm3` against 5 % of `gen.Contour.ChamberVolume_mm3` (new advisory threshold; no anchor data yet, revisit if any emerges), falling back to the pre-existing resonator-count-alone proxy (`Count > 16`) when volume isn't available (legacy/hand-built results) or is within threshold. New `AcousticDamperVolumeGateTests` (`Voxelforge.Core.Tests`) covers: the unit-level volume stamping, the gate firing on volume alone at a safe count, the gate staying silent when volume is small, and the count-fallback still firing for zero-volume legacy results — the first of these is impossible pre-fix, since the old gate body never read any volume field at all. `AcousticDamperResult.TotalVolume_mm3` added to `PublicAPI.Unshipped.txt` (RS0016). Drops the ROADMAP known-gaps burn-down line (no `physics-cascade-status.md` ledger entry existed for this one — it originated from a `RocketGates.cs` code comment, not the ledger).
...
**Optimization wiring (electric propulsion):**
- **`ResistojetObjective.Build` was missing the baseline-`Kind` guard all 5 sibling EP objectives have** (`HetObjective`, `ArcjetObjective`, `PptObjective`, `MpdObjective`, `GitObjective` each throw `ArgumentException` when `baseline.Kind` doesn't match their engine kind; `ResistojetObjective` didn't). A caller passing a HallEffect-Kind baseline got no error: `Unpack` never touches `Kind`, so it silently stayed `HallEffect`, and `ElectricPropulsionOptimization.GenerateWith`'s `switch (design.Kind)` dispatch ran the HET physics pipeline for every candidate in the "resistojet" search — the 6-dim vector being swept (`HeaterPower_W` etc.) has zero effect on HET physics, so the optimizer silently explores a flat, meaningless score surface and returns a design that's still Kind=HallEffect, not the resistojet the caller believed they were optimizing. Fixed with the same guard pattern as its siblings. New `Build_OnHallEffectBaseline_Throws` test (fails on old code: "No exception was thrown").

**Optimization wiring (nuclear):**
- **`NtrObjective.ScoreNegativeIsp` produced `NaN` instead of the `+∞` infeasibility sentinel for BimodalNtr + `BimodalMode.Electric` designs.** `NuclearOptimization.GenerateWith` deliberately NaNs `IspVacuum_s` for Electric-mode bimodal designs (no thrust in that mode), but `IsFeasible` is computed independently from gate violations, which never reference `IspVacuum_s` or `BimodalMode` — so a thermally sound Electric-mode design reached `ScoreNegativeIsp` with `IsFeasible=true` and `IspVacuum_s=NaN`. `-NaN` is `NaN`, not `+∞`; every subsequent SA acceptance/best-score comparison against a NaN `_bestScore` is `false`, so the optimizer silently stopped accepting anything for the rest of that design's run instead of correctly treating it as a "not a real thrust point" infeasibility signal (the same class of NaN-as-feasible defect A.116 fixed across the generic optimizer stack, here recurring in a pillar-specific objective). Fixed by explicitly checking `!double.IsNaN(result.IspVacuum_s)` alongside `IsFeasible`. New `Build_Evaluate_OnBimodalElectricMode_ProducesFiniteScore_NotNaN` test (fails on old code: NaN score).

**Optimization wiring (rocket) — aerospike gate coverage:**
- **`RegenChamberOptimization.Evaluate` computed the aerospike scoring sidecar but never checked the aerospike-specific feasibility gates.** `GenerateWith` populates `gen.Aerospike` (via `aerospikeBuilder.BuildPhysicsOnly`) for aerospike-topology designs, and `Evaluate`'s scoring surface already reads `gen.Aerospike.Thermal.*` extensively (peak wall T, coolant ΔP, heat load, mass — Sprint 11 Track F). But `Evaluate` calls only `FeasibilityGate.Evaluate(gen)` — the regen-family gate system — never `AerospikeFeasibility.Evaluate`, the parallel evaluator holding aerospike's 5 dedicated gates (`AEROSPIKE_PLUG_WALL_TEMP`, `AEROSPIKE_COOLANT_CAVITATION_RISK`, `AEROSPIKE_ELEMENT_CLEARANCE`, `AEROSPIKE_INJECTOR_FACE_TEMP`, `LINEAR_AEROSPIKE_ASPECT_RATIO`). Those gates were previously only reachable through the standalone `AerospikeOptimization.BuildAndEvaluate` convenience method — never through the main SA-facing `GenerateWith → Evaluate` path every aerospike design actually runs through during optimization. An aerospike design with a plug wall temperature far past the material's service limit could reach `IsFeasible = true` during SA search. Fixed by invoking `AerospikeFeasibility.Evaluate` and merging its violations into `allViolations` whenever `gen.Aerospike is not null`. New `AerospikeGateWiringTests.cs` (`Voxelforge.Core.Tests`): a hot-wall aerospike sidecar now correctly fires `AEROSPIKE_PLUG_WALL_TEMP` (fails on old code — no violation), a cool-wall sidecar stays clean, and a non-aerospike baseline is unaffected.

`ROADMAP.md` → Now criterion 1 updated: gate-registry/CLI/IO/EP/nuclear-wiring surfaces are fully swept; only the rest of rocket/airbreathing/marine optimization wiring remains for round 5 (narrower than before — `AerospikeOptimization.cs` itself and the gate-wiring gap it exposed are now closed).

### Sprint A.121 — Red-team round 4 (continued): gate registries, CLI, and IO surfaces fully swept — 4 more real defects fixed

Continues Sprint A.120's round 4 (ROADMAP → Now criterion 1). Relaunched scout agents for the surfaces round 4 hadn't yet reached — this time a moderate batch of 3 (remaining gate registries, CLI dispatch, IO schema files) rather than 5 at once, to reduce the blast radius if the earlier session-limit failure recurred. All 3 completed cleanly. Every finding personally verified by reading the actual code and, where a fix landed, temporarily reverting it to confirm the regression test fails on the pre-fix code before restoring the fix (fail-on-old / pass-on-new for every fix below).

**Found and fixed:**
- **`MarineGates.HULL_CG_CB_OFFSET_LARGE` compared a signed offset against a magnitude limit — missing `Math.Abs()`.** Both the inline comment and `MarineConstraintIds`' XML doc specify `|z_CG − z_CB| > 5% D`, but the code checked `cgCbOffset_m > cgCbLimit` directly, so any negative offset — however large — silently passed. Currently dormant (the only caller hardcodes `cgCbOffset_m: 0.0`, per `MarineResult.cs`'s documented "future variant" scope for asymmetric-payload hulls) but directly reachable via the internal `MarineGates.Evaluate` overload and would silently misfire the moment real asymmetric-payload wiring lands. New tests in `MarineGateTests.cs` call `MarineGates.Evaluate` directly (bypassing the always-0.0 pipeline callers) to exercise positive, negative, and symmetric offsets.
- **`CfdFieldExport`'s VTK point data was written z-fastest instead of the x-fastest order its own `WholeExtent`/`Piece` header declares.** The VTK ImageData file-format spec requires point ID = i + j·dimX + k·dimX·dimY (x-fastest); the code computed `idx = ((ix*Ny)+iy)*Nz+iz` (z-fastest) in both `Write()` and `WriteAerospike()`. Since the production default grid is 256×64×64 (Nx≠Nz), this isn't a harmless axis relabeling — it's a genuine flat-index mismatch that spatially scrambles every field (`solid_domain`, `fluid_domain`, `wall_temperature_K`, `velocity_init_ms`) for any real CFD export opened in ParaView/VisIt/OpenFOAM, the exact tools the file's own header names as its intended consumers. Invisible to the existing (Windows-leg) test suite because those tests only assert header text and voxel counts, never spatial placement. New `CfdFieldExportPointOrderingTests.cs` in `Voxelforge.Core.Tests` (this class is pure C#, no PicoGK — its existing tests were misplaced in the Windows-only suite) parses the raw AppendedData binary back out of a written `.vti` file and confirms a known-fluid point (the chamber axis, r=0, true for every station) lands at the VTK-spec flat index.
- **`CfdFieldExport.Write` hardcoded the gas-side wall thickness to the literal `0.8` instead of reading `ChannelSchedule.GasSideWallThickness_mm`** — the exact field it already receives as a parameter, and the same field every other consumer (`StepExport.cs`, `ChamberVoxelBuilder`, the structural/thermal solvers) reads correctly. Invisible to the existing test suite because its fixture happens to use the 0.8 mm default. For any other wall thickness, the exported solid/fluid domain boundary is silently mislocated relative to the manufactured part. Second `CfdFieldExportPointOrderingTests.cs` test uses a 2.0 mm wall and a probe point 1 mm into the true wall band to confirm the boundary tracks the design's actual value.
- **`AirbreathingForm`'s ComboBox / `--engine-kind` CLI flag silently drops 2 of 12 valid engine kinds.** `KindToIndex`/`SelectedKind` and the ComboBox item list were never extended past the original 10 kinds when `LiquidAirCycle` (LACE) and `RotatingDetonation` (RDE) were added — both fully implemented at the physics layer (registered solvers, feasibility gates, validation fixtures) but unreachable from the UI or CLI. `Voxelforge.exe --airbreathing --engine-kind RotatingDetonation` parses successfully (no error) but silently opens on Ramjet instead. An existing test (`AirbreathingFormKindCoverageTests.EveryNonSentinelKind_HasRegisteredSolver`) already pinned the count at 12 with a comment noting "adding a new kind requires a UI panel" — but the companion `DisplayNameMatchesEnumValue` theory that would have caught the actual UI-mapping gap was never extended past its original 10 `[InlineData]` cases. Fixed both the form (`AirbreathingForm.cs`) and the test (added "LACE"/"RDE" entries, now 12/12). Windows-only (`net9.0-windows`, WinForms) — math/logic-verified by reading the code and confirmed both `Voxelforge.csproj` and `Voxelforge.Tests.csproj` compile cleanly on this container (`-p:EnableWindowsTargeting=true`); full test execution awaits the Windows runner, per the established Horn-antenna-fix precedent (Sprint A.112).

**Round 4 coverage after this sprint:** the gate-registry surface (all 7 files), the CLI/`Program.cs` dispatch layer, and `Voxelforge.Core/IO/` (all files) are now fully swept. **Only the per-pillar `Optimization.cs` + `*Objective.cs` wiring surface remains** — see `ROADMAP.md` → Now criterion 1 for the exact file list.

- **`ROADMAP.md` → Now criterion 1** updated to reflect the near-complete state and the narrowed remaining scope.

### Sprint A.120 — Red-team round 4 (partial): gate-registry sweep, one real defect found

Round 4 (ROADMAP → Now criterion 1) launched 5 parallel scout agents covering the criterion's full not-yet-swept list (IO schema files, CLI dispatch, rocket/airbreathing/marine optimization wiring, EP/nuclear optimization wiring, per-pillar gate registries). All 5 hit a session-wide usage limit mid-investigation and terminated before reporting complete findings — an infrastructure failure, not a clean result. Two agents left partial leads in their last messages before dying; both were personally chased down and refuted (not bugs): a regen channel-height / dual-bell-inflection question in `RegenCoolingSolver.InterpChannelHeight` (channel height is a user-chosen 3-point schedule independent of wall shape, not derived from it — the ambiguity is in an aspirational doc comment elsewhere, not a code defect), and a film-cooling-fraction double-count suspicion in `InjectorPattern.SizePattern` (it already discounts fuel flow by the film fraction exactly once; the caller in `RegenChamberOptimization.cs` passes the undiscounted total, as expected).

With subagent spawning blocked, continued solo: re-read `NsgaIIIOptimizer.cs` and `StabilityScreening.cs` fresh (criterion 1's exit condition (c), sampled re-audit of already-swept surfaces) — both stay clean, all four A.116 optimizer fixes intact, the round-1 unreachable-Fail-branch fix intact. Then read the two largest gate-registry files in full: `RocketGates.cs` (1998 lines, ~58 gate implementations) and `MonopropGates.cs`.

**Found (documented, not fixed — data-plumbing gap, not a same-file fix):**
- **`RDE_ANNULUS_FILL_STARVED`'s inter-wave-period threshold is algebraically independent of `RdeWaveCount`.** The gate estimates the annulus circumference by inverting `DetonationWaveCount`'s own formula (`C_nominal = N·v_frac·L_cj`), since the true circumference isn't threaded through to `RegenGenerationResult`. Substituting that estimate back into `f_wave = v_wave/C` and `interWavePeriod = 1/(N·f_wave)` makes both `N` and `v_frac` cancel completely — the threshold collapses to the constant `L_cj/cjSpeed` (≈ 8.333 µs) regardless of wave count. Verified numerically: N=2 and N=8 both evaluate to exactly 8.333... µs. The Hard gate still compares `RdeAnnulusFillTime_us` correctly against that constant, but has no actual per-design discriminating power despite its Description text citing N and f_wave as if independently derived. A same-file fix isn't possible (the circularity is structural); the real fix needs the true annulus circumference threaded from `RdeCombustion` into `RegenGenerationResult`. Added a clarifying comment at the computation site in `RocketGates.cs` plus a `physics-cascade-status.md` § Documented gaps entry with the concrete N=2/N=8 demonstration.

**Round 4 is incomplete** — not a dry round, and not yet a full pass. Covered: gate-registry surface (2 of 7 files: `RocketGates.cs`, `MonopropGates.cs`). Not yet covered: `AirbreathingGateInput.cs` / `AirbreathingGates.cs` / `AirbreathingGateRegistry.cs`, `MarineGates.cs`, `NuclearGates.cs`; `Voxelforge.Core/IO/` (beyond the one file touched chasing a lead); the CLI / `Program.cs` dispatch layer; the per-pillar `Optimization.cs` + `*Objective.cs` wiring for every pillar except the one rocket file touched chasing a lead. Round 5 picks up exactly this list — see ROADMAP → Now criterion 1.

- **`ROADMAP.md` → Now criterion 1** updated to reflect round 4's actual partial coverage (not "complete," per the above); criterion 4 date/sprint references bumped to include the new ledger entry.
- **`physics-cascade-status.md`** gains the RDE finding under § Documented gaps; header refreshed to 2026-07-13, round 4 noted as cut short by the agent-infrastructure failure.
...
- **The six remaining pure-Core Wave-1 solver families gain exact closed-form regression tests in `Voxelforge.Core.Tests`**: `WindTurbineSolverTests` (½ρAV³ available power, the Gaussian-in-λ C_p cluster fit at the HAWT 0.48@λ7.5 / VAWT 0.40@λ5 anchors, the actuator-disk induction inversion 4a(1−a)² = C_p with C_T = 4a(1−a) thrust, a Betz-clamp invariant swept across λ for both kinds, and the parked cut-in/cut-out envelope), `HydroTurbineSolverTests` (ρgQH at a Three-Gorges-class Francis anchor, the per-kind cluster-η registry, the linear off-envelope head de-rating with its 30 % clamp, HE.W2 head-band kind auto-selection, and an energy-cascade P_elec ≤ P_shaft < P_hyd invariant), `ElectrolyserSolverTests` (all four sub-variants — PEM/AEM/Alkaline/SOEC — on the shared Nernst + Tafel + ohmic decomposition with their per-kind R_AS / Tafel-slope / i₀ anchors; the SOEC steam-electrolysis reference 0.923 V @ 800 °C with η_HHV > 1 endothermic operation pinned as correct-and-physical; Faraday's-law production pinned identical across kinds at equal N·I), `ChemicalReactorSolverTests` (Arrhenius k, both Damkohler forms Da₁ = kτ / Da₂ = kC₀τ, the Levenspiel CSTR/PFR/Batch closed-form conversions for first and second order, the PFR-beats-CSTR ordering, and an X-monotone-in-Da invariant), `HybridRocketSolverTests` (Marxman r_dot = a·G_ox^n at the Karabeyoglu LOX/HTPB fit, the cylindrical-port fuel-flow roll-up, the oxidiser-rich O/F drift as the port opens, the Sutton c*/C_F(ε) cluster anchors, and exact ṁ_total/thrust bookkeeping at every port radius), `AntennaSolverTests` (the Friis log-form chain at an 8 GHz X-band anchor chosen to sit exactly on both ITU table grid points, every per-topology gain anchor incl. the Kraus helix formula, the ANT.W3 kT·BW·NF sensitivity floor + achieved Eb/N₀, the P.838-3 rain + P.676-12 absorption terms, a 6-dB-per-distance-doubling FSPL invariant, and the ModulationSchemeTable round-trip + FEC-strength ordering). Every expected value hand-derived and independently recomputed before assertion; guard tests pin each design record's `ValidateSelf`. 41 new tests; full `Voxelforge.Core.Tests` suite 233/233 green locally via runtime roll-forward.
- **`ROADMAP.md` → Now §2 refreshed to the post-wave-4 state**: 16/23 → 22/23, with Stirling the only remaining family (model-fidelity-blocked, not test-writing-blocked).
...
- **One infeasible (+∞) sample permanently poisoned the GP — the optimizer then silently evaluated the unclamped zero vector for its whole remaining budget.** `BayesianOptimizer` stored raw scores (including the `+∞` hard-gate sentinel) as GP training targets; a single non-finite target makes the Cholesky solve's α all-NaN, every `Predict` mean NaN, every acquisition value NaN — and `NaN > −∞` never improves, so the candidate stayed its `new double[dim]` initializer: the **zero vector, outside the variable bounds**, evaluated and re-appended every iteration while the history looked normal. Any realistically-gated design space (mostly infeasible) triggers this with the default hard-penalty mode. Now fits the GP on the finite-score subset only (same guard `SurrogateObjective` already carried), falls back to deterministic bounded Sobol exploration while no finite sample exists, and — defence in depth — evaluates the first drawn candidate rather than the zero vector if an acquisition sweep never selects. New `BayesianInfeasiblePoisoningTests` pin "only ever evaluates inside the declared bounds" for the mixed- and all-infeasible cases.

**Optimization (multi-chain SA):**
- **`Result.BestBreakdown` could describe a different, worse design than `Result.BestParams`.** Elite migration (`SimulatedAnnealingOptimizer.MigrateFrom`) updated the receiving chain's best score/params but left `_bestBreakdown` pointing at the previous best (the comment claimed it "stays null" — it didn't). `MigrateElites` broadcasts the global best to every chain and the final tournament breaks score ties by lowest chain index, so the winner was commonly a *receiver* — surfacing a mismatched breakdown (in the `IObjective` overload: the full `EvaluationResult` with violations + physics record) as the reported winner's, including through `HybridSACmaEsOrchestrator.Run`'s `BestEvaluation`. Added a `MigrateFrom(params, score, breakdown)` overload (additive public API; `PublicAPI.Unshipped.txt` entry) that carries the donor's breakdown; the 2-arg overload now clears the stale breakdown instead of lying. New `MultiChainMigratedBreakdownTests` pin both the unit-level contract and the end-to-end `BestBreakdown.Score == BestScore` invariant.

**Optimization (NSGA-II / NSGA-III):**
- **NaN-scored candidates were classified feasible and could permanently pollute the Pareto front.** Both `ComputeConstraintViolation`s tested only `IsPositiveInfinity`; a NaN evaluation score (contract-legal: "infinite or NaN scores are surfaced verbatim") got constraint-violation 0. NaN objectives compare false both ways in `Dominates`, so a NaN individual is non-dominated forever: rank 0, straight into the returned front — reachable through the documented custom-extractor path. Both now treat any non-finite score as infeasible. New `NsgaNaNFeasibilityTests` pin NaN-free fronts for both algorithms.
- **NSGA-III could return an *empty* next generation and crash — and its singular-normalization fallback was silently wrong.** Exposed by the fix above: `ComputeNormalizedObjectives`' singular-intercept fallback took `Math.Max` over *all* rows, so a NaN row poisoned every intercept (`Math.Max(x, NaN) = NaN` → all normalised objectives NaN → the niching loop's first pick found no candidate and **aborted, returning an empty parent population** → `TournamentSelect` crashed on the next generation), while `+∞` rows drove the intercepts to `+∞` (all normalised objectives 0 — niching-by-distance quietly degenerated). The extreme-point ASF scan had the same exposure. Both loops now restrict to feasible rows (mirroring the ideal-point loop), and the niching pick falls back to the first available candidate instead of aborting the loop when every distance is NaN.

**Documented (fix needs data or a schema decision — not blind-fixed):**
- **`SobolSequence` "Joe-Kuo" direction numbers are mis-transcribed.** Dimension 3 pairs `a = 2` with `s = 4` (`m = {1,1,3,3}`), whose decoded polynomial x⁴+x²+1 = (x²+x+1)² over GF(2) is *reducible* — never a valid Sobol polynomial; the real Joe-Kuo rows are (s=3, a=2, m={1,1,1}) and (s=4, a=1, m={1,1,3,3}) — one row's `a` fused with the next row's `m`; dims 4–7 match no Joe-Kuo row, and the dims ≥ 8 fallback is neither Sobol nor Halton despite the header claims. Everything stays deterministic and in [0, 1), so the blast radius is warmup-coverage *quality* (MultiChain/Bayesian seeding), not correctness. The fix is transcribing the genuine new-joe-kuo-6 table plus a property test (m_i odd, m_i < 2^i, primitive polynomial) — deferred to a dedicated change rather than hand-typing constants unverified.
- **`DesignPersistence` stores enums as raw ordinals with no `JsonStringEnumConverter`** — contradicting the v24→v25 migration comment that claims string serialisation. Today's enums are append-only so round-trip is correct, but any insertion/reorder silently remaps every saved design's topology/damper/igniter with no migration hook and no validation catch. The fix wants a deliberate schema bump (v32) with a numeric→name migration — a design-intent decision, so documented here instead of patched.
- Minor, tracked with the above: `NsgaIIIOptimizer`'s constructor probes the objective once at the raw lower-bounds corner to count objectives (side-effects stateful wrappers; not counted in `TotalEvaluations`), and `CmaEsOptimizer` carries a dead `_c_1` assignment immediately overwritten (cosmetic).
...
**Test coverage (Linux CI backfill):**
- **Six more pure-Core Wave-1 solver families gain exact closed-form regression tests in `Voxelforge.Core.Tests`** (net9.0 → the reliable `core (linux)` leg), lifting family coverage from 4/23 to 10/23: `FlywheelSolverTests` (I = α·m·R², E = ½Iω², thin-rim hoop stress, burst speed/SF, √SoC derating, bearing-drag auto-discharge τ = 1/f invariant), `WingSparSolverTests` (rect/hollow-box/circular section properties, cantilever root moment, UDL tip deflection, load-factor linearity, AS.W2 elliptical 0.75/0.65 factors), `HydrogenStorageSolverTests` (real-gas ρ = P·M/(Z·R·T) with the Z = 1 + 6e-4·P fit, LH₂/metal-hydride densities, boil-off ṁ = Q/h_fg, sub-linear-in-P density invariant), `HeatPipeSolverTests` (per-limit throughputs, R = L/(k_eff·A), the #548-C 4 kW/1 m/25 mm sodium anchor ΔT ≈ 45.3 K < 50 K, sodium's governing-limit-is-sonic ordering, fluid auto-selection boundaries), `ThermoelectricGeneratorSolverTests` (ZT figure-of-merit efficiency, exact hot-side energy balance, strict η < Carnot bound, #548-E segmented-cascade ≥ max(η_h, η_l) invariant), `SolarCollectorSolverTests` (Hottel-Whillier-Bliss budget, exact-zero at stagnation + clamp beyond, stagnation-temperature inversion, zero-irradiance guard). Every expected value hand-derived and independently recomputed before assertion; guard tests pin each design record's `ValidateSelf`. 43 new tests; full `Voxelforge.Core.Tests` suite 145/145 green (verified locally via runtime roll-forward, see below).
- **`CLAUDE.md`: the .NET-10 container can now *run* the net9.0 test suites, not just compile them.** `DOTNET_ROLL_FORWARD=LatestMajor dotnet test …` rolls the runtime forward at launch without touching the pinned `global.json` — replaces the previous "compile check only, let CI run the tests" guidance for the cross-platform suites (CI remains the authoritative signal).

### Sprint A.113 — Docs: strategic-alignment refresh — v0.1.0 release-readiness criteria + physics-cascade ledger

A planning-alignment pass grounded in verified repository state (git tags, CI runs, Pages deployments, per-leg test counts, the restored issue backlog) rather than remembered state. Docs-only; no code change.

- **`ROADMAP.md` "Now" rewritten to the post-public reality.** The repository has been public since 2026-06-17 with Pages deploying green since 2026-06-18 — but no v0.1.0 tag exists, and the two post-public red-team rounds (Sprints A.111/A.112) each surfaced 10+ real correctness bugs that green CI missed. The bare "cut the tag" step is therefore replaced with five explicit release-readiness criteria: (1) a red-team dry round (zero new hard-gate-class defects); (2) Linux coverage parity for pure-Core physics — only 4 of 23 Wave-1 solver families have Linux-leg regression tests today, the other 19 are tested solely in the Windows-only `Voxelforge.Tests` (~67 % of all test methods run only on the often-offline self-hosted runner); (3) one green Windows-leg run to execute the never-run A.111/A.112 regression tests; (4) the known-gaps ledger current at the cut; (5) ADR-037 release mechanics. Extends — does not amend — ADR-037 D7, whose original preconditions are resolved. Also: strategic-framing status line refreshed, runner-offline SPOF distinguished from the runner-parallelization item, and tracker-hygiene notes added (#2 satisfied by observation — Pages is enabled and deploying; #46 one-third fixed by A.112; #30/#32 VFA003 diagnostic-ID collision).
- **`physics-cascade-status.md` refreshed after going two sprints stale** (header still said 2026-05-24, violating its own 1-sprint staleness rule; it predated both red-team rounds). New **Documented gaps** section records the calibration-blocked defects deliberately not auto-fixed, each with file pointers and a fix path: the Crocco n-τ stability screen's dead Fail branch (`STABILITY_FAIL` cannot fire), VASIMR unbounded low-η_i Isp, bimodal-NTR Hybrid reactor-power double-count, the HET beam-current↔I_d model coupling (now gate-guarded by `HET_POWER_BALANCE_VIOLATED`), the Stirling free-piston 10–100× over-prediction, and the two remaining #46 antenna voxel defects (Helical, Patch). New **Windows-leg validation pending** section queues the A.111/A.112 fixes whose regression tests target the offline self-hosted runner and have never executed (horn SDF, NTR nozzle length, setup-wizard default, VFD013/VFD016 analyzer tests) so the runner's first green run is read as fresh signal. Pre-2026-06 Resolved entries dropped per the header rule (git history retains them); A.111/A.112 recorded as the current Resolved reference.
...
**Test coverage (Linux CI backfill):**
- **Safety-critical pure-Core pillar solvers ran only on the offline Windows runner.** Several Wave-1 solver families (`PressureVesselSolver`, `SpacecraftRadiatorSolver`, `EpsilonNtuSolver`) are PicoGK/WinForms-free but had unit coverage only in `Voxelforge.Tests` (`net9.0-windows`), so the reliable Linux `core` CI leg validated none of their physics. Added cross-platform tests in `Voxelforge.Core.Tests` — `PressureVesselSolverTests` (hoop/axial/von-Mises stress, burst pressure, safety factor, internal volume, thin-wall sizing, Lamé thick-wall), `SpacecraftRadiatorSolverTests` (Stefan-Boltzmann emission, two-sided area doubling, required-area sizing), and `EpsilonNtuSolverTests` (counterflow ε-NTU effectiveness + 1-D fin efficiency) — all exact closed-form assertions, no behaviour change.
