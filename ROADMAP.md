...
1. **Red-team round 4 — nearly complete; one narrow surface left.** Rounds 1–3 each found real defects (round 3, Sprint A.116: 4 in the optimizer stack). Concrete exit rule (unchanged from A.119): **(a)** a round finishes the first full pass over the not-yet-swept surfaces, **(b)** every finding from that pass is fixed-or-documented, **(c)** a sampled re-audit of already-swept surfaces comes back clean. **(c) satisfied** (A.120: NSGA-III niching/normalization and the combustion-stability composite screen freshly re-read, stay clean). Round 4 spanned three sprints (A.120, A.121, A.122) and repeated infrastructure interruptions (subagent session-limit deaths mid-round, each followed by either a clean recovery or a solo continuation): the gate-registry surface (all 7 files), the CLI/`Program.cs` dispatch layer (incl. `AirbreathingForm.cs` and all 5 `*.StlExporter/Program.cs` entry points), `Voxelforge.Core/IO/` (all 8 files + the Nuclear IO variant), and the electric-propulsion + nuclear optimization-orchestration/objective-wiring surface are now **fully swept**. Eight real hard-gate-class defects found and fixed (fail-on-old/pass-on-new tests, Linux-verified except the one Windows-only fix): `MarineGates.HULL_CG_CB_OFFSET_LARGE` missing `Math.Abs()`; `CfdFieldExport`'s VTK point data written in the wrong axis order (spatially scrambled every exported field whenever Nx≠Nz); `CfdFieldExport` hardcoding 0.8 mm gas-side wall thickness instead of reading the design's actual value; `AirbreathingForm`'s `--engine-kind` mapping silently dropping 2 of 12 engine kinds (Windows-only to execute, math/logic-verified by hand); `ResistojetObjective.Build` missing the baseline-`Kind` guard all 5 sibling EP objectives have (a mismatched-Kind baseline silently optimized the wrong physics pipeline — every candidate scored identically since `Unpack` never varies `Kind`); `NtrObjective.ScoreNegativeIsp` producing `NaN` (not the intended `+∞` infeasibility sentinel) for any feasible BimodalNtr+Electric-mode design, since `-NaN` is still `NaN` — this NaN-poisoned the SA optimizer's acceptance logic for the design's whole run; and — found personally chasing a dying agent's lead — `RegenChamberOptimization.Evaluate` computing the aerospike scoring sidecar (`gen.Aerospike`) but never invoking `AerospikeFeasibility.Evaluate`, so aerospike topology's 5 dedicated hard/advisory gates (plug wall temp, coolant cavitation, injector clearance, injector face temp, linear aspect ratio) were structurally unreachable from the main SA-facing optimization path — only the standalone `AerospikeOptimization.BuildAndEvaluate` convenience method ever checked them. Plus one documented-not-fixed: `RDE_ANNULUS_FILL_STARVED`'s inter-wave-period formula is algebraically wave-count-independent (A.120; needs a data-plumbing change, not a same-file fix). Three dead-end leads chased and refuted (`RegenCoolingSolver` channel-height/dual-bell-inflection; `InjectorPattern.SizePattern` film-fraction double-count; both A.120). **Remaining surface** (round 5, narrower than before): the rest of rocket + airbreathing + marine optimization-orchestration/objective wiring — `RegenChamberOptimization.cs` beyond the aerospike-gate fix, `RegenObjective.cs`, `AirbreathingOptimization.cs` + 5 objectives, `MarineOptimization.cs` + 2 objectives (`AerospikeOptimization.cs` and the aerospike-gate gap it exposed are now fully covered). Plus the per-pillar `*.Voxels` builders (Windows-only, low priority until criterion 3). Sandbox-runnable — no Windows runner needed.
2. **Linux coverage parity for pure-Core physics — SATISFIED for every portable family.** **22 of 23** Wave-1 pillar solver families now have Linux-leg regression tests in `Voxelforge.Core.Tests` (was 4 at the start of this pass; 16 before wave 4) — HeatExchanger, Radiator, Tankage, Refrigeration, Flywheel, Aerostructures, HydrogenStorage, HeatPipe, Thermoelectric, SolarThermal, Pump, Compressor, ElectricMotor, Battery, Photovoltaic, PowerGen(PemFuelCell), and (wave 4, Sprint A.118) WindTurbine, Hydroelectric, Electrolyser (4 sub-variants: AEM/PEM/Alkaline/SOEC — count as one family), Antenna, ChemicalReactor, HybridRocket. **Only Stirling remains**, and it is a special case: no defensible fixture exists yet because the Wave-1 cluster fit over-predicts free-piston output 10–100× (see Done → Stirling deferred; MEP-model refinement is a prerequisite, not a test-writing task). Voxel/UI suites stay Windows-bound by nature — this criterion covers what *can* move.
...
4. **Known-gaps ledger current at the cut — SATISFIED as of Sprint A.120.** [`physics-cascade-status.md`](Voxelforge/docs/physics-cascade-status.md) (header: 2026-07-13) records every calibration-blocked gap: Crocco stability screen, VASIMR Isp ceiling, bimodal-Hybrid power split, HET beam-current coupling, Stirling MEP, the A.116 finds (`SobolSequence` Joe-Kuo direction numbers, `DesignPersistence` raw-ordinal enums), and the A.120 find (`RDE_ANNULUS_FILL_STARVED`'s wave-count-independent formula — a data-plumbing gap, not a calibration one, but likewise not a same-file fix). Re-check the 1-sprint staleness rule immediately before cutting the tag; release notes reference this ledger per ADR-037 D6.
...
Everything below is fully recorded in [`physics-cascade-status.md`](Voxelforge/docs/physics-cascade-status.md) § Documented gaps (Now §4 confirms the ledger is current) but has no forward plan beyond "documented" — this gives the accumulated residue a named home instead of leaving it purely archival:

- Crocco n-τ stability screen sign flip + recalibration against validated-engine fixtures.
- HET beam-current/discharge-current coupling + BPT-4000 / SPT-100 / HiVHAc recalibration.
- Bimodal NTR Hybrid-mode reactor power split (design-intent decision + fixture recalibration).
- VASIMR Isp ceiling calibrated against VX-200 data.
- Stirling MEP-model refinement (STR.W2) — prerequisite for the coverage-parity fixture deferred in Now §2.
- `SobolSequence` Joe-Kuo direction-number table transcription (quality-only).
- `DesignPersistence` schema-v32 enum-string migration decision.
...
Tracker state (2026-07-13): 32 of the 33 restored issues remain open after this pass's hygiene cleanup (#2 closed). Notes:
