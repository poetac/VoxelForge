...
1. **Red-team round 4 completes the first full pass — tag on that, not on an open-ended "dry round."** Rounds 1–3 each found real defects (round 3, Sprint A.116: 4 in the optimizer stack — GP poisoning, a cross-chain breakdown/score mismatch, NaN-as-feasible in both NSGA variants, and an NSGA-III empty-generation crash): every *new* surface has yielded something, while re-swept surfaces have stayed clean. At that hit rate, "zero new defects" doesn't reward a clean codebase — it rewards running out of new places to look. Concrete exit rule: **(a)** round 4 finishes the first full pass over the surfaces below, **(b)** every finding from that pass is fixed-or-documented (physics-cascade-status.md for anything calibration-blocked), and **(c)** a sampled re-audit of 2–3 already-swept surfaces (e.g. the optimizer stack, rocket physics) comes back clean. Swept so far: rocket/marine/nuclear/EP/airbreathing physics formulas, system-integration/economics, the Wave-1 energy/thermal solver bodies, the optimizer/IO/gate-mechanics/memory-envelope layer. **Not yet swept** (round 4 scope): `Voxelforge.Core/IO/` beyond the persistence-migration mechanics already audited clean, the per-pillar `*.Voxels` builders (Windows-only, low priority until criterion 3), the CLI/`Program.cs` dispatch layer, the per-pillar `Optimization.cs` orchestration files (`RegenChamberOptimization`, `MarineOptimization`, etc. — distinct from the generic optimizers just hardened), and the per-pillar gate-registry variants (`AirbreathingGateRegistry`, `MarineGates`, etc. — beyond the generic `GateRegistry` mechanics already checked). Sandbox-runnable — no Windows runner needed.
...
4. **Known-gaps ledger current at the cut — SATISFIED as of Sprint A.117.** [`physics-cascade-status.md`](Voxelforge/docs/physics-cascade-status.md) (header: 2026-07-11) records every calibration-blocked gap: Crocco stability screen, VASIMR Isp ceiling, bimodal-Hybrid power split, HET beam-current coupling, Stirling MEP, plus the A.116 finds — the mis-transcribed `SobolSequence` Joe-Kuo direction numbers (quality-only, warmup-coverage degradation, not correctness) and `DesignPersistence`'s raw-ordinal enum serialization (needs a schema-v32 decision, tracked as a design question, not a ledger gap). Re-check the 1-sprint staleness rule immediately before cutting the tag — A.118 (wave-4 coverage backfill) added no new gaps, so the ledger is still current going into A.119, but anything landing after needs a fresh pass; release notes reference this ledger per ADR-037 D6.
