...
- Test *execution* on the .NET-10 container needs **both** relaxations together, not the env var alone: `global.json` still has to be temp-relaxed to `latestMajor` (the `dotnet test` command itself resolves against `global.json` before it ever launches a test host) *and* `DOTNET_ROLL_FORWARD=LatestMajor` has to be set (the spawned `testhost.dll` process does its own separate runtime resolution and ignores `global.json`) — e.g. relax `global.json`, then `DOTNET_ROLL_FORWARD=LatestMajor dotnet test Voxelforge.Core.Tests/… -c Release --no-build`, then `git checkout -- global.json`. Skipping either half fails differently: no `global.json` relaxation → `dotnet test` itself refuses to start (`A compatible .NET SDK was not found`); no env var → `dotnet test` starts fine but `testhost.dll` dies (`You must install or update .NET to run this application`). Verified on the full `Voxelforge.Core.Tests` suite (most recently 238/238, Sprint A.122). `core (linux)` CI remains the authoritative signal; this is for fast local iteration.
...
**Some containers have no local .NET SDK at all (found 2026-07-19).** Not every cloud/web session container matches the ".NET 10 present, relax `global.json` to compile-check" story above — one observed instance had `dotnet` missing entirely (`command not found`, no `/usr/share/dotnet`, nothing installable via apt/snap), **and** the egress proxy denied `builds.dotnet.microsoft.com` with a 403 policy rejection (confirmed via `curl -sS "$HTTPS_PROXY/__agentproxy/status"` → `recentRelayFailures: connect_rejected`), so the official `dotnet-install.sh` script couldn't be fetched either. Per the proxy's own guidance (`/root/.ccr/README.md` § "403/407 from the proxy"): do not retry or route around a policy denial. In this configuration there is **no way to build or run tests locally** — check `dotnet --version` (or just try a build) early in a session before assuming the documented recipe applies, and if it's absent, fall back entirely to pushing a branch + reading the `core (linux)` / other `*(linux)` job logs via the GitHub MCP tools (`get_job_logs` with `return_content: true`) as the sole build+test oracle. This is slower (a few minutes' round-trip per iteration, and CI in this repo has repeatedly returned in well under a minute) but is also the *authoritative* target environment anyway, so it's not a lesser check — just a slower one. Say so explicitly in the PR body rather than silently omitting the usual "ran the build" verification step.
...
**GitHub MCP gotcha (found 2026-07-18):** `add_issue_comment` unconditionally appends a
"_Generated by Claude Code_" footer to the posted comment body — this happens below the
tool boundary and is **not** avoidable by omitting it from the `body` argument, and there
is no comment-edit or comment-delete tool in this MCP server's surface to remove it
afterward (verified: neither exists in the tool list). `issue_write` (create/update) and
`update_pull_request`, by contrast, are not tainted and are freely re-editable — this is
why PR-body footers have been stripped successfully all session via `update_pull_request`
while issue-comment footers cannot be. **Consequence:** route durable, attribution-
sensitive content (implementation guides, specs) through an issue/PR **body**
(`issue_write` / `update_pull_request`), not a comment, whenever practical. Issues #75–#84
each carry a guide comment with this footer live and uncorrectable with current tooling;
left as-is rather than compounding it with more tainted comments.
