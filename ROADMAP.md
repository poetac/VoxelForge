...
1. **Red-team round 4 — started, cut short by an infrastructure failure; resumes as round 5.** Rounds 1–3 each found real defects (round 3, Sprint A.116: 4 in the optimizer stack — GP poisoning, a cross-chain breakdown/score mismatch, NaN-as-feasible in both NSGA variants, and an NSGA-III empty-generation crash): every *new* surface has yielded something, while re-swept surfaces have stayed clean. Concrete exit rule (unchanged from A.119): **(a)** a round finishes the first full pass over the not-yet-swept surfaces, **(b)** every finding from that pass is fixed-or-documented, **(c)** a sampled re-audit of already-swept surfaces comes back clean. Round 4 (Sprint A.120) got partway there before 5 parallel scout agents hit a session-wide usage limit mid-investigation. **(c) satisfied**: NSGA-III niching/normalization and the combustion-stability composite screen were freshly re-read and stay clean. Of the not-yet-swept list below, round 4 fully covered the two largest gate-registry files — `RocketGates.cs` (1998 lines, ~58 gates) and `MonopropGates.cs` — and found one real hard-gate-class defect: `RDE_ANNULUS_FILL_STARVED`'s inter-wave-period formula is algebraically wave-count-independent (collapses to a constant regardless of `RdeWaveCount` — documented in physics-cascade-status.md and code-commented, not fixed, since the real fix needs a data-plumbing change, not a same-file one). Two partial leads left behind by the dying agents were personally chased down and refuted (a regen channel-height/dual-bell-inflection question in `RegenCoolingSolver.cs`, a film-cooling-fraction double-count suspicion in `InjectorPattern.SizePattern` — neither panned out). **Still not swept** (round 5 scope): `AirbreathingGateInput.cs` / `AirbreathingGates.cs` / `AirbreathingGateRegistry.cs`, `MarineGates.cs`, `NuclearGates.cs` (rest of the gate-registry surface); `Voxelforge.Core/IO/` beyond the persistence-migration mechanics and the one file touched chasing a lead; the CLI/`Program.cs` dispatch layer (untouched); the per-pillar `Optimization.cs` orchestration + `*Objective.cs` wiring files for every pillar except the one rocket file touched chasing a lead; the per-pillar `*.Voxels` builders (Windows-only, low priority until criterion 3). Sandbox-runnable — no Windows runner needed.
