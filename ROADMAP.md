...
1. **Red-team round 4 — nearly complete; one surface left.** Rounds 1–3 each found real defects (round 3, Sprint A.116: 4 in the optimizer stack). Concrete exit rule (unchanged from A.119): **(a)** a round finishes the first full pass over the not-yet-swept surfaces, **(b)** every finding from that pass is fixed-or-documented, **(c)** a sampled re-audit of already-swept surfaces comes back clean. **(c) satisfied** (A.120: NSGA-III niching/normalization and the combustion-stability composite screen freshly re-read, stay clean). Round 4 spanned two sprints (A.120, A.121) and two infrastructure interruptions (a subagent session-limit death mid-round, then a clean recovery): the gate-registry surface (all 7 files — `RocketGates.cs`, `MonopropGates.cs`, `AirbreathingGateInput/Gates/GateRegistry.cs`, `MarineGates.cs`, `NuclearGates.cs`), the CLI/`Program.cs` dispatch layer (incl. `AirbreathingForm.cs` and all 5 `*.StlExporter/Program.cs` subprocess entry points), and `Voxelforge.Core/IO/` (all 8 files + the Nuclear IO variant) are now **fully swept**. Five real hard-gate-class defects found and fixed (fail-on-old/pass-on-new tests, Linux-verified except the one Windows-only fix): `MarineGates.HULL_CG_CB_OFFSET_LARGE` missing `Math.Abs()` (silently cleared any negative CG/CB offset); `CfdFieldExport`'s VTK point data written z-fastest instead of the x-fastest order its own header declares (spatially scrambled every exported field whenever Nx≠Nz — the production default is 256×64×64); `CfdFieldExport` hardcoding 0.8 mm gas-side wall thickness instead of reading the design's actual value; `AirbreathingForm`'s `--engine-kind`/ComboBox mapping silently dropping 2 of 12 engine kinds (LiquidAirCycle, RotatingDetonation — both fully solver-backed, unreachable UI/CLI only, Windows-only to execute but math/logic-verified by hand per the established Horn-antenna-fix precedent). Plus one documented-not-fixed: `RDE_ANNULUS_FILL_STARVED`'s inter-wave-period formula is algebraically wave-count-independent (A.120; needs a data-plumbing change, not a same-file fix). Two dead-end leads chased and refuted (A.120: `RegenCoolingSolver` channel-height/dual-bell-inflection; `InjectorPattern.SizePattern` film-fraction double-count). **Only remaining surface**: the per-pillar `Optimization.cs` orchestration + `*Objective.cs` wiring files for every pillar except the one rocket file (`InjectorPattern.cs`) touched chasing a lead — `RegenChamberOptimization.cs`/`AerospikeOptimization.cs`/`RegenObjective.cs` (rocket), `AirbreathingOptimization.cs` + 5 objectives, `MarineOptimization.cs` + 2 objectives, `ElectricPropulsionOptimization.cs` + 6 objectives, `NuclearOptimization.cs` + `NtrObjective.cs`. Plus the per-pillar `*.Voxels` builders (Windows-only, low priority until criterion 3). Sandbox-runnable — no Windows runner needed.
